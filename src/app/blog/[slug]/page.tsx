...
/**
 * Produce route parameters for static generation from the blogPosts collection.
 *
 * @returns An array of parameter objects of the form `{ slug: string }`, where each `slug` is derived from a post's `slug` with the "/blog/" prefix removed.
 */
...
/**
 * Estimate the reading time for a blog post based on its text content.
 *
 * @param post - The blog post object; uses `post.content` when it's a string, otherwise uses `post.description`
 * @returns A string like `"3 min read"` with a minimum of `"1 min read"`
 */
...
/**
 * Render the blog post page for the given route slug.
 *
 * Looks up the post by `/blog/{slug}`, computes related posts and previous/next navigation, and returns the page UI for that post. If no matching post is found, triggers a 404 response.
 *
 * @param params - A promise that resolves to an object containing the route `slug`.
 * @returns The React element for the blog post page.
 */
