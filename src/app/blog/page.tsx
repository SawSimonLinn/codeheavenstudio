...
/**
 * Estimate the reading time for a blog post.
 *
 * Uses the post's `content` when it's a string, otherwise falls back to `description`.
 *
 * @param post - The blog post object whose text will be measured (`content` or `description`)
 * @returns A string like "`<n> min read`" representing the estimated minutes to read, with a minimum of `1`
 */
...
/**
 * Renders the interactive blog listing page with a hero, sticky tag filter, featured post, post grid, and stats strip.
 *
 * @returns The React element for the blog page containing header, footer, tag-filterable posts (featured + rest), and summary statistics.
 */
...
/**
 * Renders a clickable card summarizing a blog post for use in the posts grid.
 *
 * @param post - The blog post object to display (provides imageUrl, title, description, date, tags, slug, and aiHint).
 * @param index - Zero-based position of the post in the list; used to render the two-digit index badge.
 * @returns The JSX element for a linked post card. 
 */
function PostCard({
  post,
  index,
